...
import com.yzh.voicemanager.util.CommonDefine;
import com.yzh.voicemanager.util.FileUtils;
import com.yzh.voicemanager.util.TimeMethod;
import com.yzh.voicemanager.util.TimeSpan;
import com.yzh.voicemanager.util.Utils;
...
public class VoiceManage {
	...
	private String filePath = "";// 录音所存放的位置  "/com.yzh.voicemanager/audio"
	private View v = null;
	...
	private MediaPlayer.OnCompletionListener mPlayCompetedListener 
		...
		public void onCompletion(MediaPlayer mp) {
			...
			mIVPlaOperate.setImageResource(R.drawable.voice_play);
			...
			try {
				...
				case CommonDefine.MSG_TIME_INTERVAL:
					if (mDeviceState == CommonDefine.MEDIA_STATE_RECORD_DOING) {
						...
						ts = TimeSpan.timeSpanSecond(current / 1000);
	...
	public VoiceManage(Activity act, View view, String filePath) {
		mActivity = act;
		v = view;
			...
			mIVPlaOperate.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (mDeviceState == CommonDefine.MEDIA_STATE_PLAY_DOING) {
						...
						mIVPlaOperate.setImageResource(R.drawable.voice_pause);
						mHandler.removeMessages(CommonDefine.MSG_TIME_INTERVAL);
		...
		if (mSBPlayProgress != null) {
			mSBPlayProgress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
				...
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
					...
					TimeSpan ts = TimeSpan.timeSpanSecond(progress / 1000);
					mTVPlaCurrent.setText(String.format("%02d:%02d:%02d", 
							ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));	
				}
	...
	public void sessionRecord(boolean init) {
		...
		mIVRecOperate.setImageResource(R.drawable.voice_record);
	...
	public void clickRecordFinish() {
		try {
			...
			if(TimeSpan.timeSpanSecond(mRecTimeSum).mSpanSecond == 0){
...
//						REQUEST_CODE, Activity.RESULT_OK, intent);
				}
			}	 				
	...
	public void sessionPlay(boolean init, String path) {
		...
		try {
			...
			if (prepareMedia(mMediaPlayer, path)) {
				...
				TimeSpan ts = TimeSpan.timeSpanSecond(mMediaPlayer.getDuration() / 1000);
	...
	private File getOutputVoiceFile(ArrayList<File> list) {
		String mMinute1 = Utils.getTime();
		String recFilePath = Utils.recAudioPath(filePath);
		File recDirFile = Utils.recAudioDir(recFilePath);
	...
	private File prepareRecorder(MediaRecorder mr, boolean start) {
		...
		try {
			String path = Utils.recAudioPath(filePath);
			recFile = new File(path, Utils.getTime() + ".amr");
